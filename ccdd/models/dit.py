...
class MDIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
  def __init__(self, config, vocab_size: int):
    super().__init__()
    if type(config) == dict:
      config = omegaconf.OmegaConf.create(config)
    ...
    self.register_buffer("logit_bias", torch.full((1, 1, 1), 0.0))
    self.latent_output_layer = DDitFinalLayer(config.model.hidden_size, self.latent_dim, config.model.cond_dim)
  ...
  def forward(self, indices, z, sigma, attention_mask=None, c_cont=None):
    ...
    z = self.latent_embed(z, c)
    x = x + z

    for i in range(len(self.blocks)):
      x = self.blocks[i](x, rotary_cos_sin, c, seqlens=None)
    
    z = self.latent_output_layer(x, c)
...
class MMDDiTBlockWithMask(nn.Module):
  def __init__(self, dim, n_heads, cond_dim, mlp_ratio=4, dropout=0.1):
    ...
    # Self-attention layers for both modalities
    self.norm1_x = LayerNorm(dim)
    self.norm1_z = LayerNorm(dim)
    self.attn_qkv_x = nn.Linear(dim, 3 * dim, bias=False)
    self.attn_qkv_z = nn.Linear(dim, 3 * dim, bias=False)
    self.attn_out_x = nn.Linear(dim, dim, bias=False)
    self.attn_out_z = nn.Linear(dim, dim, bias=False)
    
    # Cross-attention layers
    self.norm_cross_x = LayerNorm(dim)
    self.norm_cross_z = LayerNorm(dim)
    self.cross_attn_q_x = nn.Linear(dim, dim, bias=False)  # query from x
    self.cross_attn_kv_z = nn.Linear(dim, 2 * dim, bias=False)  # key, value from z
    self.cross_attn_q_z = nn.Linear(dim, dim, bias=False)  # query from z
    self.cross_attn_kv_x = nn.Linear(dim, 2 * dim, bias=False)  # key, value from x
    self.cross_attn_out_x = nn.Linear(dim, dim, bias=False)
    self.cross_attn_out_z = nn.Linear(dim, dim, bias=False)

    # MLP layers for both modalities
    self.norm2_x = LayerNorm(dim)
    self.norm2_z = LayerNorm(dim)
    ...
    # AdaLN modulation for both modalities
    self.adaLN_modulation_x = nn.Linear(cond_dim, 9 * dim, bias=True)  # 9 = 6 + 3 for cross-attn
    self.adaLN_modulation_z = nn.Linear(cond_dim, 9 * dim, bias=True)  # 9 = 6 + 3 for cross-attn
    ...
    with torch.no_grad():
      self.adaLN_modulation_x.bias.data[2::9] = 1.0  # gate_msa
      self.adaLN_modulation_x.bias.data[5::9] = 1.0  # gate_cross
      self.adaLN_modulation_x.bias.data[8::9] = 1.0  # gate_mlp
      
      self.adaLN_modulation_z.bias.data[2::9] = 1.0  # gate_msa
      self.adaLN_modulation_z.bias.data[5::9] = 1.0  # gate_cross
      self.adaLN_modulation_z.bias.data[8::9] = 1.0  # gate_mlp
  ...
  def _compute_attention(self, qkv, attention_mask, batch_size, seq_len):
    """Helper function to compute attention with optional masking"""
    if has_flash_attn and attention_mask is None:
      # Use flash attention only when no attention mask is needed
      ...
      return rearrange(attn_out, '(b s) h d -> b s (h d)', b=batch_size)
    else:
      # Use standard attention with optional masking
      q, k, v = qkv[:, :, 0].transpose(1, 2), qkv[:, :, 1].transpose(1, 2), qkv[:, :, 2].transpose(1, 2)
      ...
      return rearrange(attn_out, 'b h s d -> b s (h d)', b=batch_size)

  def _compute_cross_attention(self, q, kv, attention_mask_q, attention_mask_kv, batch_size, seq_len):
    """Helper function to compute cross-attention with optional masking"""
    q = rearrange(q, 'b s (h d) -> b h s d', h=self.n_heads)
    k, v = rearrange(kv, 'b s (two h d) -> b s two h d', two=2, h=self.n_heads).chunk(2, dim=2)
    k = k.squeeze(2).transpose(1, 2)  # b h s d
    v = v.squeeze(2).transpose(1, 2)  # b h s d
    
    if attention_mask_q is not None or attention_mask_kv is not None:
      # Create cross-attention mask
      if attention_mask_q is not None and attention_mask_kv is not None:
        # mask[i,j] = 1 if query position i is valid AND key position j is valid
        mask_q = attention_mask_q.bool()  # (B, S)
        mask_kv = attention_mask_kv.bool()  # (B, S)
        cross_mask = mask_q.unsqueeze(-1) & mask_kv.unsqueeze(-2)  # (B, S, S)
        cross_mask = cross_mask.unsqueeze(1)  # (B, 1, S, S)
      elif attention_mask_q is not None:
        mask_q = attention_mask_q.bool().unsqueeze(-1)  # (B, S, 1)
        cross_mask = mask_q.expand(-1, -1, seq_len).unsqueeze(1)  # (B, 1, S, S)
      else:
        mask_kv = attention_mask_kv.bool().unsqueeze(-2)  # (B, 1, S)
        cross_mask = mask_kv.expand(-1, seq_len, -1).unsqueeze(1)  # (B, 1, S, S)

      float_mask = torch.zeros(cross_mask.shape, device=q.device, dtype=q.dtype)
      float_mask = float_mask.masked_fill(~cross_mask, -1e9)
  ...
  def forward(self, x, z, rotary_cos_sin, c, attention_mask=None, seqlens=None, c_cont=None):
    ...
    # Generate modulation parameters for both modalities
    c_cont = c_cont if c_cont is not None else c
    modulation_x = self.adaLN_modulation_x(c)[:, None].chunk(9, dim=2)
    modulation_z = self.adaLN_modulation_z(c_cont)[:, None].chunk(9, dim=2)
    
    (shift_msa_x, scale_msa_x, gate_msa_x, shift_cross_x, scale_cross_x, gate_cross_x,
     shift_mlp_x, scale_mlp_x, gate_mlp_x) = modulation_x
    (shift_msa_z, scale_msa_z, gate_msa_z, shift_cross_z, scale_cross_z, gate_cross_z,
     shift_mlp_z, scale_mlp_z, gate_mlp_z) = modulation_z

    # === Self-attention for both modalities ===
    x_skip, z_skip = x, z
    
    # Self-attention for x
    x_norm = modulate_fused(self.norm1_x(x), shift_msa_x, scale_msa_x)
    qkv_x = self.attn_qkv_x(x_norm)
    qkv_x = rearrange(qkv_x, 'b s (three h d) -> b s three h d', three=3, h=self.n_heads)
    cos, sin = rotary_cos_sin
    qkv_x = apply_rotary_pos_emb(qkv_x, cos.to(qkv_x.dtype), sin.to(qkv_x.dtype))
    
    attn_x = self._compute_attention(qkv_x, attention_mask, batch_size, seq_len)
    x = bias_dropout_scale_fn(self.attn_out_x(attn_x), None, gate_msa_x, x_skip, self.dropout)
    
    # Self-attention for z
    z_norm = modulate_fused(self.norm1_z(z), shift_msa_z, scale_msa_z)
    qkv_z = self.attn_qkv_z(z_norm)
    qkv_z = rearrange(qkv_z, 'b s (three h d) -> b s three h d', three=3, h=self.n_heads)
    qkv_z = apply_rotary_pos_emb(qkv_z, cos.to(qkv_z.dtype), sin.to(qkv_z.dtype))
    
    attn_z = self._compute_attention(qkv_z, attention_mask, batch_size, seq_len)
    z = bias_dropout_scale_fn(self.attn_out_z(attn_z), None, gate_msa_z, z_skip, self.dropout)

    # === Cross-attention ===
    x_cross_skip, z_cross_skip = x, z
    
    # Cross-attention: x attends to z
    x_cross_norm = modulate_fused(self.norm_cross_x(x), shift_cross_x, scale_cross_x)
    z_cross_norm = modulate_fused(self.norm_cross_z(z), shift_cross_z, scale_cross_z)
    
    q_x = self.cross_attn_q_x(x_cross_norm)
    kv_z = self.cross_attn_kv_z(z_cross_norm)
    cross_attn_x = self._compute_cross_attention(q_x, kv_z, attention_mask, attention_mask, batch_size, seq_len)
    x = bias_dropout_scale_fn(self.cross_attn_out_x(cross_attn_x), None, gate_cross_x, x_cross_skip, self.dropout)
    
    # Cross-attention: z attends to x
    q_z = self.cross_attn_q_z(z_cross_norm)
    kv_x = self.cross_attn_kv_x(x_cross_norm)
    cross_attn_z = self._compute_cross_attention(q_z, kv_x, attention_mask, attention_mask, batch_size, seq_len)
    z = bias_dropout_scale_fn(self.cross_attn_out_z(cross_attn_z), None, gate_cross_z, z_cross_skip, self.dropout)

    # === MLP for both modalities ===
    # MLP for x
    x = bias_dropout_scale_fn(
      self.mlp_x(modulate_fused(self.norm2_x(x), shift_mlp_x, scale_mlp_x)),
      None, gate_mlp_x, x, self.dropout)
    
    # MLP for z
    z = bias_dropout_scale_fn(
      self.mlp_z(modulate_fused(self.norm2_z(z), shift_mlp_z, scale_mlp_z)),
      None, gate_mlp_z, z, self.dropout)
...
class MMDIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
  def __init__(self, config, vocab_size: int):
    super().__init__()
    if type(config) == dict:
      config = omegaconf.OmegaConf.create(config)
  ...
  def forward(self, indices, z, sigma, attention_mask=None, c_cont=None):
    ...
    rotary_cos_sin = self.rotary_emb(x)  # Can use either x or z since same seq_len

    # Process through mixture blocks
    for i in range(len(self.blocks)):
      x, z = self.blocks[i](x, z, rotary_cos_sin, c, attention_mask, seqlens=None, c_cont=c_cont)
    ...
    logits_x = logits_x.scatter_add(-1, indices.unsqueeze(-1), 
                                   self.logit_bias_x.to(logits_x.dtype).expand_as(logits_x))
  ...
  def forward_single_modality(self, indices, sigma, modality='x', attention_mask=None):
    ...
    if modality == 'x':
      ...
      for i in range(len(self.blocks)):
        x, z = self.blocks[i](x, z, rotary_cos_sin, c, attention_mask, seqlens=None)
      ...
      raise ValueError(f"Invalid modality: {modality}. Must be 'x' or 'z'.")
